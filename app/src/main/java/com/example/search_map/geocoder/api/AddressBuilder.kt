...
class AddressBuilder {
    ...
    fun parseResult(json: String): List<Address> { //результат
    ...
    @Throws(JSONException::class)
    private fun parseAddress(jsonObject: JSONObject): Address { //адрес
    ...
    @Throws(JSONException::class)
    private fun getAddressComponents(jsonComponents: JSONArray): List<AddressComponent> { //получение компонентов адреса
    ...
    private fun getFullAddress(street: String?, number: String?): StringBuilder { //получение полного адреса
